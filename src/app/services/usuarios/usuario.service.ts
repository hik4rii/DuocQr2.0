...
getUser(user: string, pass: string): Observable<Iusuario | null> {
  const userMatch = this.usuarios.find((aux) => aux.usuario === user && aux.contrasena === pass);
  if (userMatch) {
    return of(userMatch);
  } else {
...
getUserTypeByEmailAndPassword(email: string, pass: string): Observable<any | null> {
  const usuario = this.usuarios.find((user) => user.correo === email && user.contrasena === pass);
